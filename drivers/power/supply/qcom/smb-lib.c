...
int smblib_set_icl_current(struct smb_charger *chg, int icl_ua)
{
	...
	smblib_dbg(chg, PR_MISC, "set icl %d\n", icl_ua);
				...
				union power_supply_propval *val)
{
	...
	case INHIBIT_CHARGE:
		if (POWER_SUPPLY_HEALTH_WARM == pval.intval || POWER_SUPPLY_HEALTH_OVERHEAT == pval.intval)
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
		else
...
#define HVDCP3_CURRENT_UA_F2		2500000
					...
					int usb_current)
{
	...
	if (usb_current > 0 && usb_current < USBIN_500MA) {
		smblib_err(chg, "usb_current < USBIN_500MA = %d\n", usb_current);
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_FLOAT) {
		if (usb_current == -ETIMEDOUT) {
			...
			/*if typec mode is audio adapter not vote 1A*/
...
#ifdef CONFIG_CHARGER_BQ25910_SLAVE
		*total_current_ua = HVDCP3_CURRENT_UA_F2;
#else
					      ...
					      bool rising)
{
...
#if 0
	/*add for e2 support huawei scp charger*/
	...
	if (apsd_result->bit & QC_2P0_BIT) {
		...
		if (!chg->unstandard_hvdcp) {
			vote(chg->usb_icl_votable, HVDCP2_ICL_VOTER, true, HVDCP2_CURRENT_UA);
			rc = smblib_force_vbus_voltage(chg, FORCE_9V_BIT);
			if (rc < 0)
				pr_err("Failed to force 9V\n");
		} else {
...
#ifdef CONFIG_CHARGER_BQ25910_SLAVE
		current_ua = HVDCP3_CURRENT_UA_F2;
		if (!delayed_work_pending(&chg->dp_dm_pulse_work))
			schedule_delayed_work(&chg->dp_dm_pulse_work, msecs_to_jiffies(500));
#else
					      ...
					      bool rising, bool qc_charger)
{
	...
	smblib_dbg(chg, PR_OEM, "IRQ: smblib_handle_hvdcp_check_timeout %s\n",
		   rising ? "rising" : "falling");
}
					    ...
					    bool rising)
{
	...
	/* the APSD done handler will set the USB supply type */
	cancel_delayed_work_sync(&chg->hvdcp_detect_work);
	smblib_dbg(chg, PR_OEM, "IRQ: hvdcp-detect-done %s\n",
		   rising ? "rising" : "falling");
}
...
void smblib_usb_typec_change(struct smb_charger *chg)
{
	...
	if (chg->typec_status[3] & TYPEC_VBUS_ERROR_STATUS_BIT)
		smblib_dbg(chg, PR_OEM, "IRQ: vbus-error\n");
...
int smblib_init(struct smb_charger *chg)
{
	...
	INIT_DELAYED_WORK(&chg->connector_health_work, smblib_connector_health_work);
